...
try:
    ...
    init(autoreset=True)
    R, G, Y, C, W = Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.CYAN, Style.RESET_ALL
except ImportError:
    # fallback nếu không cài colorama
    R = '\033[31m'
    G = '\033[32m'
    Y = '\033[33m'
    C = '\033[36m'
...
def process_proxies():
    ...
    def worker(proxy):
        ...
        if alive:
            print(f"\n{random.choice(colors)}⚡ [✓] Live Proxy: {proxy}{W}")
        else:
            print(f"\n{R}☠ [x] Dead Proxy: {proxy}{W}")
...
def auto_ghost_mode(proxies):
    if not proxies:
        print(f"{R}[!] No proxies available. Auto Ghost Mode cannot start.{W}")
        return
